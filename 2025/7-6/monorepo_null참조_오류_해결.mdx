...
최근 프로덕트의 수가 증가하고, 프론트엔드 개발자 인력이 부족한 상황이 발생하였다. 이로 인해 모노레포 및 디자인 시스템을 도입하고자 의사결정하였다.

cacro를 통해 모노레포를 구현하는 과정에서 [package > component](외부 패키지의 컴포넌트)를 어플리케이션 중 하나에서 사용할 때에 아래와 같이 null 참조 오류가 발생하였다.

# ❏ 문제 분석

> 어플리케이션 빌드 이후 외부 패키지(component 등)를 사용할 때, **외부 모듈에서 사용하는 React Hook이 정상적으로 동작하지 않음**
